	...
	switch($do){
		...
		case 'fillMagasin':
			...
			if(isset($_POST["province"])){
				(isset($_POST["province"]) && $_POST["province"]=='TOUS' ? $where='' : $where=" WHERE magasin.IDPROVINCE ='".$_POST["province"]."'");
				//SQL
				
				$sql  = "SELECT * FROM magasin  $where ORDER BY magasin.SER_NOM ASC;";
				try {
					$cnx = new PDO(DBD, DBUSER, DBPWD, array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8")); //Connexion to database
				}
				catch (PDOException $error) { //Treat error
		...
		case 'next': //Par produit
			...
			while($row = $query->fetch(PDO::FETCH_ASSOC)){
				...
				$Declassement = StockProduitParNature($row['CODE_PRODUIT'], 'PERTE', $whereAll);
				...
				//Declassement
				$PDeclassement = StockProduitParNature($row['CODE_PRODUIT'], 'PERTE', $whereAll);
...
{
(isset($_POST['region']) && $_POST['region'] ? $chx ='1' : $region = '');
(isset($_POST['province']) && $_POST['province'] ? $chx ='2' : $province = '');
(isset($_POST['service']) && $_POST['service'] ? $chx ='3' : $service = '');
}

$userName = getField('LOGIN',$_SESSION['GL_USER']['LOGIN'],'LOGIN','compte');
$ilang=getCodelangue($userName);
if ($choix ='0'){
if($ilang=='1' && $ilang!='') { $_SESSION['DATA_ETAT']['choix'] = 'Tous les sites (National)';}
if($ilang=='2' && $ilang!='') { $_SESSION['DATA_ETAT']['choix'] = 'All the sites (National)';}
if($ilang=='3' && $ilang!='') { $_SESSION['DATA_ETAT']['choix'] = 'Todos os sítios (Nacional)';}
}	
	$d1 = date('Y-m-d');
...
//	
			...
			$whereAllcmm=$whereAll." AND mouvement.MVT_DATE>='$d3' AND mouvement.MVT_DATE<='$d1'";
//echo $whereAll.'   ------   '.$whereAllcmm;
//break;
			...
			while($row = $query->fetch(PDO::FETCH_ASSOC)){
				...
				$qteEntreec1 = StockProduit($row['CODE_PRODUIT'], 'E', $whereAllcmm);
				$qteSortiec1 = StockProduit($row['CODE_PRODUIT'], 'S', $whereAllcmm);
				$cmm=($qteEntreec1['QTE'] - $qteSortiec1['QTE'])/3;
...
//	$cmm = ($qte3['QTE'] - $qteDecl3['QTE'])/3;
		...
		case 'rapdetailentree':
			...
			while($row = $query->fetch(PDO::FETCH_ASSOC)){
				...
				'qteentre'=>$row['MVT_QUANTITE'],'transfertsort'=>$transfetSort['QTE'], 'transfertent'=>$transfetEnt['QTE'],'bonsortie'=>$bonsortie['QTE'],
		...
		case 'rapmvtfournisseur':
			...
			AND livraison.CODE_FOUR='$idfournisseur' $whereAll GROUP BY MVT_REFLOT ORDER BY produit.PRD_LIBELLE ASC; ";
		...
		case 'rapprdperime':
			...
			if(count($produit)==0 ){
				...
				if ($categorie=='TOUS'){
					...
					//Produit
					$sql = "SELECT CODE_PRODUIT FROM produit
					INNER JOIN souscategorie ON (produit.CODE_SOUSCATEGORIE LIKE souscategorie.CODE_SOUSCATEGORIE) WHERE souscategorie.CODE_CATEGORIE LIKE $categorie ORDER BY PRD_LIBELLE ASC;";
					$query =  $cnx->prepare($sql); //Prepare the SQL
					$query->execute(); //Execute prepared SQL => $query
					while($row = $query->fetch(PDO::FETCH_ASSOC)){
						$in .="'".$row['CODE_PRODUIT']."', ";
			...
			$qteE = "SELECT produit.CODE_PRODUIT, produit.PRD_LIBELLE, mouvement.MVT_TYPE, mouvement.MVT_DATEPEREMP, Sum(mouvement.MVT_QUANTITE) AS SommeDeMVT_QUANTITE, mouvement.MVT_REFLOT FROM mouvement INNER JOIN produit ON mouvement.CODE_PRODUIT = produit.CODE_PRODUIT GROUP BY produit.CODE_PRODUIT, produit.PRD_LIBELLE, mouvement.MVT_TYPE, mouvement.MVT_DATEPEREMP, mouvement.MVT_REFLOT HAVING mouvement.MVT_TYPE='E' AND mouvement.MVT_DATEPEREMP<Now()";
//echo $qteE;
//break;


//			"SELECT * FROM mouvement INNER JOIN produit ON (mouvement.CODE_PRODUIT LIKE produit.CODE_PRODUIT)
//			WHERE mouvement.MVT_TYPE LIKE 'E' AND  MVT_DATEPEREMP<NOW() $in ORDER BY produit.PRD_LIBELLE ASC; ";
		...
		case 'rapmvtstock':
			...
			while($row = $query->fetch(PDO::FETCH_ASSOC)){
				...
				$ecart   =	$inventmoins['QTE'] + $inventplus['QTE'];
				$rest = $entree - ($sortie);
				$declass   =	$Declassement['QTE'];

				//echo  'Ent'.$entree.' Sort'.$sortie.'<br>';
				array_push($_SESSION['DATA_ETAT']['ligne'], array('codeproduit'=>$row['CODE_PRODUIT'], 'produit'=>addslashes($row['PRD_LIBELLE']),
				'qteentree'=>$entree,'qtesortie'=>$sortie, 'declass'=>$declass, 'stocks'=>$rest,'unite'=>$row['ID_UNITE']));
			}
			$_SESSION['DATA_ETAT']['nbreLigne'] =$query->rowCount();
			//print_r($_SESSION['DATA_ETAT']);
			...
			switch($typemouvement){
				...
				case 7://'7'=>'PERTES'
					...
					while($row = $query->fetch(PDO::FETCH_ASSOC)){
						...
						AND mouvement.ID_EXERCICE=".$row['ID_EXERCICE']." AND mouvement.MVT_NATURE LIKE 'PERTE'";
						$query2 =  $cnx->prepare($sqlCoresp); //Prepare the SQL
						$query2->execute(); //Execute prepared SQL => $query
		...
		case 'raprupture':
			...
			(isset($_POST['categorie']) && $_POST['categorie']!='0'	? $categorie = $_POST['categorie'] 	: $categorie 	= '');
			(isset($_POST['souscategorie']) && $_POST['souscategorie']!='0'	? $souscategorie = $_POST['souscategorie'] 	: $souscategorie 	= '');
			(isset($_POST['nommagasin'])  ?  $nommagasin =$_POST['nommagasin'] : $nommagasin='');
		...
		case 'rapportmensuel': //Par produit
				...
				if($_POST['par']=='PRD'){
					...
					while($row = $query->fetch(PDO::FETCH_ASSOC)){
						...
						//Declassement
						$PDeclassement = StockProduitParNaturePeriode($row['CODE_PRODUIT'], 'PERTE', $wherePeriode);
						...
						if ($PEntree!=0 || $PSortie!=0 || $rest!=0){
							array_push($_SESSION['DATA_ETAT']['ligne'], array('codeproduit'=>$row['CODE_PRODUIT'], 'produit'=>addslashes($row['PRD_LIBELLE']), 'livraison'=>$Livraison['QTE'],
							'qteentre'=>$entree,'transfertsort'=>$transfetSort['QTE'], 'transfertent'=>$transfetEnt['QTE'], 'bonsortie'=>$bonsortie['QTE'],
							'qteperime'=>$qeperime, 'reportentree'=>$reportEntree['QTE'], 'reportsortie'=>$reportSortie['QTE'],'declass'=>$Pperte,
...
$choix='Tous les sites (National)';
(isset($_POST['region']) && $_POST['region'] ? $region =$_POST['region'] : $region = '');
(isset($_POST['province']) && $_POST['province'] ? $province =$_POST['province'] : $province = '');
$choix1=getSiteCent($region);
$choix2=getSiteFour($province);
if ($region=='0') 
...
$choix='Tous les sites fournisseurs';
(isset($_POST['province']) && $_POST['province'] ? $province =$_POST['province'] : $province = '');
(isset($_POST['service']) && $_POST['service'] ? $service =$_POST['service'] : $service = '');
$choix2=getSiteFour($province);
$choix3=getSiteBénéf($service);
...
$_SESSION['DATA_ETAT']['choix'] = $choix;
			...
			WHERE mouvement.MVT_TYPE LIKE 'S' AND mouvement.MVT_NATURE LIKE 'PERTE'  $whereAll ORDER BY produit.PRD_LIBELLE ASC; ";
			$query =  $cnx->prepare($sql); //Prepare the SQL
			$query->execute(); //Execute prepared SQL => $query
			...
			while($row = $query->fetch(PDO::FETCH_ASSOC)){
				...
				//echo  'Ent'.$entree.' Sort'.$sortie.'<br>';
				array_push($_SESSION['DATA_ETAT']['ligne'], array('codeproduit'=>$row['CODE_PRODUIT'],'reflot'=>$row['MVT_REFLOT'],'produit'=>addslashes($row['PRD_LIBELLE']),
				'nature'=>$nature, 'qte'=>$row['MVT_QUANTITE'], 'unite'=>$row['ID_UNITE'], 'prix'=>$row['PRD_PRIXREVIENT']));
			}
			$_SESSION['DATA_ETAT']['nbreLigne'] =$query->rowCount();
			//print_r($_SESSION['DATA_ETAT']);
...
{
(isset($_POST['region']) && $_POST['region'] ? $localit =$region : $region = '');
(isset($_POST['province']) && $_POST['province'] ? $localit =$province : $province = '');
(isset($_POST['service']) && $_POST['service'] ? $localit =$service : $service = '');
}
...
if($ilang=='1' && $ilang!='') { $_SESSION['DATA_ETAT']['choix'] = 'Tous les sites (National)';}
	...
	if($ilang=='3' && $ilang!='') { $_SESSION['DATA_ETAT']['choix'] = 'Todos os sítios (Nacional)';}
			...
			if(isset($_POST['service']) && $_POST['service']!='0' && $_POST['province']!='0' && $_POST['region']!='0'){
				//Un magasin
				$where=" mouvement.CODE_MAGASIN LIKE '".$_POST['service']."' AND ";
				$in=" CODE_MAGASIN LIKE '".$_POST['service']."' AND ";
			}
			else if(isset($_POST['service']) && $_POST['service']=='0' && $_POST['province']!='0' && $_POST['region']!='0'){
				//Tous les magasin de la province
				$sql ="SELECT magasin.* FROM `magasin` WHERE IDPROVINCE LIKE '".$_POST['province']."'";
...
//				$in ='';
				...
				//Tous les magasin de la region (National)
				$sql ="SELECT magasin.CODE_MAGASIN, magasin.IDPROVINCE, province.IDREGION FROM magasin INNER JOIN province ON magasin.IDPROVINCE = province.IDPROVINCE WHERE province.IDREGION LIKE '".$_POST['region']."'";
	...
	(isset($_POST['datefin']) && $_POST['datefin']!=''  ? $where .="mouvement.MVT_DATE <= '".addslashes(mysqlFormat(trim($_POST['datefin'])))."' AND " 	: $where .="");
			...
			while($row = $query->fetch(PDO::FETCH_ASSOC)){
				...
				if ($rest!=0 && $rest>$max) {
					...
					'qteentre'=>$entree,'transfertsort'=>$transfetSort['QTE'], 'transfertent'=>$transfetEnt['QTE'], 'bonsortie'=>$bonsortie['QTE'],
					'qteperime'=>$qeperime, 'reportentree'=>$reportEntree['QTE'], 'reportsortie'=>$reportSortie['QTE'],'declass'=>$Declassement['QTE'],
					'qtesortie'=>$sortie, 'ecart'=>$ecart,  'stocks'=>$rest,  'max'=>$max,'unite'=>$row['ID_UNITE'],'cmm'=>$cmm));
					$Nbre++;
				}
...
$_SESSION['DATA_ETAT']['datefin'] = $datefin;
			...
			if(isset($_POST['service']) && $_POST['service']!='0' && $_POST['province']!='0' && $_POST['region']!='0'){
				//Un magasin
				$where="  mouvement.CODE_MAGASIN LIKE '".$_POST['service']."' AND ";
				$in="  mouvement.CODE_MAGASIN LIKE '".$_POST['service']."' AND ";
			}
			else if(isset($_POST['service']) && $_POST['service']=='0' && $_POST['province']!='0' && $_POST['region']!='0'){
				//Tous les magasin de la province
				$sql ="SELECT magasin.* FROM `magasin` WHERE IDPROVINCE LIKE '".$_POST['province']."'";
...
//			$_SESSION['DATA_ETAT']['DATEPEREMPTION'] = $datePeremption;
			...
			$Nbre = 0;
			while($row = $query->fetch(PDO::FETCH_ASSOC)){
				$whereAll=$whereAll0." AND mouvement.CODE_PRODUIT LIKE '".$row['CODE_PRODUIT']."' AND mouvement.MVT_REFLOT LIKE '".$row['MVT_REFLOT']."'";
				$tProduit = StockProduitPerime($row['CODE_PRODUIT'], $type='E',  $whereAll);
				$qeperime = $tProduit['QTE'];
				$Livraison = StockProduitParNature($row['CODE_PRODUIT'], 'LIVRAISON', $whereAll);
				$bonsortie = StockProduitParNature($row['CODE_PRODUIT'], 'BON DE SORTIE', $whereAll);
				...
				array_push($_SESSION['DATA_ETAT']['ligne'], array('reflot'=>$row['MVT_REFLOT'], 'codeproduit'=>$row['CODE_PRODUIT'], 'produit'=>addslashes($row['PRD_LIBELLE']),
				'dateperemption'=>$row['MVT_DATEPEREMP'],'stock'=>$stock));
				$Nbre++;
			}
			$_SESSION['DATA_ETAT']['nbreLigne'] = $Nbre; //$query->rowCount();
