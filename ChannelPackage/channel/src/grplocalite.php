...
if(isset($_GET['do']) && $_GET['do']=="search") {
	$where ="";
	(isset($_POST['codegrplocalite']) && $_POST['codegrplocalite']!='' 	? 	$where .="groupelocalite.CODE_CATEGORIE LIKE '".addslashes(trim($_POST['codegrplocalite']))."' AND " 	: $where .="");
	(isset($_POST['grplocalite']) && $_POST['grplocalite']!='' 			? 	$where .="groupelocalite.GRPLOC_LIBELLE LIKE '".addslashes(trim($_POST['grplocalite']))."%' AND " 	: $where .="");
	(isset($_POST['dependance']) && $_POST['dependance']!='0' 			? 	$where .="groupelocalite.GRPLOC_LIEN LIKE '".addslashes(trim($_POST['dependance']))."' AND " 	: $where .="");

	if($where != '')  {$where = substr($where,0, strlen($where)-4);
		$_SESSION['WHERE'] = $where;
	}
	elseif($_SESSION['WHERE'] !='') {$where = $_SESSION['WHERE'];}
	$link ='grplocalite.php?selectedTab=par&do=search';
	$retour = ligneConTypeLocalite($where,'','', $page, $_SESSION['GL_USER']['ELEMENT']); //$where, $order, $sens, $page=1, $nelt
}
else {
	$link ='grplocalite.php?selectedTab=par';
	$retour = ligneConTypeLocalite('','','', $page, $_SESSION['GL_USER']['ELEMENT']); //$where, $order, $sens, $page=1, $nelt
}

$pageLengh = pageLengh($_SESSION['GL_USER']['ELEMENT']);
$ligneConCategories = $retour['L'];
//Page
    ...
    <td width="85%" height="80%" align="left" valign="top"><table width="100%"  border="0" cellspacing="0" cellpadding="2">
      ...
      <tr>
        <td><table width="100%" border="0" align="left" cellpadding="1" cellspacing="1">
		  ...
		  <form name="ListingForm" action="dbgrplocalite.php" method="POST">
          <tr>
            <td width="43%"><?php echo  sousMenuDroitSansVlider($page='grplocalite', $tab='param', $droitMAJ); ?></td>
            ...
            <td colspan=3 align="left" valign="top">
                ...
                <table width="100%" border="0" cellpadding="2" cellspacing="1" class="tableBorder">
                  ...
                  <tr class="header2">
                    ...
                    <td width="40%" height="25" align="left" valign="middle" class="header2Bg">&nbsp;&nbsp;Libell&eacute; type de localite</td>
                    <td width="40%" height="25" align="left" valign="middle" class="header2Bg">&nbsp;&nbsp;D&eacute;pendance</td>
                   </tr>
                  <!-- Begin Table row -->
                  <?php echo $ligneConCategories;?>
                  <!-- End Table row -->
                </table>
                <!-- End Data Table -->
           ...
           <table border="0" align="right" cellpadding="5" cellspacing="0" class="botBorder">
            <tbody>
              <tr align="left" valign="top">
                <td><input name="Enregistrer2" type="button" class="button" id="Enregistrer2"  value='<?php echo getlang(187); ?>'   onClick="OpenBigWin('printgrplocalite.php','');"></td>
                </tr>
            </tbody>
          </table>
            <!-- Fin Affichage -->
