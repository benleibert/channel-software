...
switch($do){
	//Log in User
	case 'add':
		(isset($_POST['codenature']) && $_POST['codenature']!=''  	? $codenature 	= trim($_POST['codenature']) 	: $codenature = '');
		(isset($_POST['nature']) && $_POST['nature']!=''  			? $nature 		= trim($_POST['nature']) 		: $nature = '');
		(isset($_POST['sens']) && $_POST['sens']!=''  				? $sens 		= trim($_POST['sens']) 			: $sens = '');

		//SQL
		$sql  = "INSERT INTO `nature` (`CODE_NATURE` ,`NAT_LIBELLE` ,`SENS_MVT`) VALUES ('".addslashes($codenature)."', '".addslashes($nature)."' , '".addslashes($sens)."')";
