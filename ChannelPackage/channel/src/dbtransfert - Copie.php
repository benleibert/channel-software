...
switch($do){
	case 'next':
		...
		if($nature==1) {header('location:addtransfert1.php?selectedTab=bds');}
		elseif($nature==2){ header('location:addtransfert2.php?selectedTab=bds');}
		break;

	case 'trs1': //Connait la source
		(isset($_POST['exercice']) && $_POST['exercice']!=''			? $exercice 	= trim($_POST['exercice']) 			: $exercice = '');
		(isset($_POST['datetransfert']) && $_POST['datetransfert']!=''  ? $datetransfert = trim($_POST['datetransfert']) 	: $datetransfert = '');
		...
		//(isset($_POST['camion']) && $_POST['camion']!=''  				? $camion 		 = trim($_POST['camion']) 			: $camion = '');
	...
	case 'trs2': //Connait la destination
		(isset($_POST['exercice']) && $_POST['exercice']!=''			? $exercice 	= trim($_POST['exercice']) 			: $exercice = '');
		(isset($_POST['datetransfert']) && $_POST['datetransfert']!=''  ? $datetransfert = trim($_POST['datetransfert']) 		: $datetransfert = '');
		(isset($_POST['reftransfert']) && $_POST['reftransfert']!=''  ? $reftransfert = trim($_POST['reftransfert']) 	: $reftransfert = '');
		(isset($_POST['nature1']) && $_POST['nature1']!='0'  ? $nature = trim($_POST['nature1']) 	: $nature = '');
		...
		$_SESSION['DATA_TRS']=array(
			...
			'magasin_dest'=>$_SESSION['GL_USER']['MAGASIN'],
			'libelleetat'=>$libelleetat,
			//'camion'=> $camion,
			'nbreLigne'=>$nbreLigne
			);
		//Etape 2
		header('location:addtransfert22.php?selectedTab=bds');

		break;

	//TRANSFERT SORTANT

	case 'add1':  //TRANSFERT SORTANT
		(isset($_POST['exercice']) && $_POST['exercice']!=''			? $exercice 	= trim($_POST['exercice']) 			: $exercice = '');
		(isset($_POST['datetransfert']) && $_POST['datetransfert']!=''  ? $datetransfert = trim($_POST['datetransfert']) 		: $datetransfert = '');
		(isset($_POST['reftransfert']) && $_POST['reftransfert']!=''  	? $reftransfert = trim($_POST['reftransfert']) 	: $reftransfert = '');
		(isset($_POST['nature1']) && $_POST['nature1']!='0'  ? $nature = trim($_POST['nature1']) 	: $nature = '');
		...
		$numauto = myDbLastId('transfert', 'ID_TRANSFERT', $magasin)+1;
		$codeTrs = "$numauto/$magasin";

		//Insert
		$sql  = "INSERT INTO `transfert` (`CODE_TRANSFERT`, `CODE_MAGASIN`, `ID_EXERCICE`, `REF_TRANSFERT`, `ID_TRANSFERT`,
		`MAG_CODE_MAGASIN`, `TRS_DATE`, `TRS_NATURE`, `TRS_VALIDE`, `TRS_DATEVALID`, `TRS_LIBELLE`)
 		VALUES ('".addslashes($codeTrs)."','".addslashes($magasin)."','".addslashes($exercice)."', '".addslashes($reftransfert)."',
 		'".addslashes($numauto)."','".addslashes($magasin_dest)."', '".addslashes($datetransfert)."','$nature', '$statut',
		...
		for($i=1; $i<=$_SESSION['DATA_TRS']['nbreLigne']; $i++){
			...
			if($codeproduit!='' && $produit!='' && $qte!='') {
				...
				VALUES ('".addslashes($codeDetTrs)."', '".addslashes($codeproduit)."',	'".addslashes($magasin)."', '".addslashes($codeTrs)."','".addslashes($numautoDetTrs)."',
				...
				'".addslashes($codeproduit)."',	'".addslashes($magasin)."',	'".addslashes($numautoMvt)."', '".addslashes($codeTrs)."',
	...
	case 'add2':
		...
		`MAG_CODE_MAGASIN`, `TRS_DATE`, `TRS_NATURE`, `TRS_VALIDE`, `TRS_DATEVALID`, `TRS_LIBELLE`)
 		...
 		'".addslashes($numauto)."','".addslashes($magasin)."', '".addslashes($datetransfert)."','$nature', '$statut',
		'".addslashes(date('Y-m-d H:i:s'))."', '".addslashes($libelleetat)."')";
	...
	case 'update1':
		...
		//Insert
		$sql  = "UPDATE `transfert` SET `TRS_LIBELLE`='".addslashes($libelleetat)."' , 	`MAG_CODE_MAGASIN`='".addslashes($magasin_dest)."' ,
	...
	case 'update2':
		...
		//Insert
		$sql  = "UPDATE `transfert` SET `TRS_LIBELLE`='".addslashes($libelleetat)."' ,
		`MAG_CODE_MAGASIN`='".addslashes($magasin_srce)."' ,`ID_EXERCICE`='".addslashes($exercice)."' ,`TRS_DATE`='".addslashes($datetransfert)."' ,
		`TRS_VALIDE`= '$statut',`TRS_DATEVALID`='".date('Y-m-d H:i:s')."'  WHERE  CODE_TRANSFERT LIKE '".addslashes($xid)."' ; ";
		...
		for($i=1; $i<=$_SESSION['DATA_TRS']['nbreLigne']; $i++){
			...
			if($oldcodeproduit!='' && $codeproduit!='' && $produit!='' && $qte!='') {
				$sql1 .="UPDATE `dettransfert` SET `CODE_PRODUIT`='".addslashes($codeproduit)."' ,`TRS_PRDQTE`='".addslashes($qte)."' ,
				`CODE_MAGASIN`='".addslashes($magasin)."', `TRS_UNITE`='".addslashes($unite)."', AND `TRS_MONLOT`='".addslashes($monlot)."'
				WHERE CODE_DETTRANSFERT='".addslashes($code_dettransfert)."';";
				...
				`MVT_DATE`='".addslashes($datetransfert)."' ,`MVT_QUANTITE`='".addslashes($qte)."' ,`MVT_UNITE`='".addslashes($unite)."' ,`MVT_NATURE`='TRANSFERT ENTRANT',`MVT_VALID`='$statut',
				MVT_MONLOT='".addslashes($monlot)."' WHERE CODE_PRODUIT='".addslashes($oldcodeproduit)."' AND `MVT_NATURE`='TRANSFERT ENTRANT' AND `ID_SOURCE`='".addslashes($xid)."' AND `MVT_TYPE`='S'; ";
			...
			elseif($oldcodeproduit=='' && $codeproduit!='' && $produit!='' && $qte!='') {
				...
				$sql1 .="INSERT INTO `dettransfert` (`ID_TRANSFERT`, `CODE_PRODUIT`, `TRS_PRDQTE`, `TRS_PRDRECU`, `TRS_UNITE`,
				`TRS_REFLOT`, `TRS_MONLOT`, `TRS_DATEPEREMP`, `TRS_PV`,`CODE_MAGASIN`) 	VALUES ('".addslashes($insert_id)."', '".addslashes($codeproduit)."',
				'".addslashes($qte)."' , '".addslashes($qte)."' ,'".addslashes($unite)."', '".addslashes($reflot)."','".addslashes($monlot)."' ,
				'".addslashes(mysqlFormat($dateperemp))."',	'".addslashes($prix)."' ,	'".addslashes($magasin)."'); ";

				$sql2 .="INSERT INTO `mouvement` (`ID_EXERCICE` ,`CODE_PRODUIT` ,`ID_SOURCE` ,`CODE_MAGASIN` ,`MVT_DATE` ,`MVT_TIME` ,`MVT_QUANTITE` ,
				`MVT_UNITE` ,`MVT_NATURE` ,	`MVT_VALID`,`MVT_TYPE`, MVT_REFLOT, MVT_DATEPEREMP,`MVT_PV`,MVT_MONLOT) VALUES ('".addslashes($exercice)."','".addslashes($codeproduit)."',
				'".addslashes($insert_id)."', '".addslashes($magasin)."', '".addslashes($datetransfert)."' ,'".addslashes(date('H:i:s'))."',
				'".addslashes($qte)."' , '".addslashes($unite)."' ,'TRANSFERT ENTRANT','$statut', 'E',
				'".addslashes($reflot)."' ,'".addslashes(mysqlFormat($dateperemp))."','".addslashes($prix)."', '".addslashes($monlot)."') ; ";
				//array_push($_SESSION['DATA_TRS']['ligne'], array('codeproduit'=>$codeproduit, 'produit'=>$produit, 'qte'=>$qte, 'unite'=>$unite));
			}
		}
	...
	case 'validate2':
		...
		//Insert
		$sql  = "UPDATE `transfert` SET `TRS_VALIDE`= '$statut',`TRS_DATEVALID`='".date('Y-m-d H:i:s')."'  WHERE CODE_TRANSFERT LIKE '".addslashes($xid)."'; ";
		...
		for($i=1; $i<=$_SESSION['DATA_TRS']['nbreLigne']; $i++){
			...
			if($oldcodeproduit!='' && $codeproduit!='' && $produit!='' && $qte!='') {
				$sql1 .="UPDATE `mouvement` SET `MVT_VALID`='1', `MVT_TYPE`='E',`MVT_DATEVALID`='".date('Y-m-d H:i:s')."'
				 WHERE `CODE_PRODUIT`='".addslashes($oldcodeproduit)."' AND ID_SOURCE LIKE '".addslashes($xid)."' AND MVT_NATURE='TRANSFERT ENTRANT';";
			}
		}
		if (($sql1 !='')) {
			$query =  $cnx->prepare($sql1); //Prepare the SQL
	...
	case 'detail':
		...
		'magasin_srce'=>$row['CODE_MAGASIN'],
		'magasin_dest'=>$row['MAG_CODE_MAGASIN'],
	...
	case 'journal':
		...
		//LIGNES MOUVEMENT
		 $sql = "SELECT * FROM `mouvement` WHERE MVT_NATURE LIKE 'TRANSFERT%' AND ID_SOURCE = '".addslashes($xid)."' ORDER BY CODE_PRODUIT ASC";
		$query =  $cnx->prepare($sql); //Prepare the SQL
		$query->execute(); //Execute prepared SQL => $query
...
switch($myaction){
	...
	case 'addline1':
		...
		for($i=1; $i<=$_SESSION['DATA_TRS']['nbreLigne']; $i++){
			...
			if($codeproduit!='' && $produit!='' && $qte!='') array_push($_SESSION['DATA_TRS']['ligne'], array('code_dettransfert'=>$iddetbonsortie,'monlot'=>$monlot,'codeproduit'=>$codeproduit, 'prix'=>$prix,'produit'=>$produit,  'qte'=>$qte, 'unite'=>$unite, 'reflot'=>$reflot, 'dateperemp'=>$dateperemp));
		}
		//Add line
		$_SESSION['DATA_TRS']['nbreLigne'] +=1;
		//print_r($_SESSION['DATA_DOT']);
		header('location:edittransfert1.php?selectedTab=bds');
		break;
	...
	case 'addline2':
		...
		(isset($_POST['datetransfert']) && $_POST['datetransfert']!=''  ? $datetransfert = trim($_POST['datetransfert']) 		: $datetransfert = '');
		(isset($_POST['reftransfert']) && $_POST['reftransfert']!=''  ? $reftransfert = trim($_POST['reftransfert']) 	: $reftransfert = '');
		(isset($_POST['nature1']) && $_POST['nature1']!=''  ? $nature = trim($_POST['nature1']) 	: $nature = '');
		(isset($_POST['magasin_srce']) && $_POST['magasin_srce']!=''  	? $magasin_srce  = trim($_POST['magasin_srce']) 		: $magasin_srce = '');
		//(isset($_POST['magasin_dest']) && $_POST['magasin_dest']!=''  	? $magasin_dest  = trim($_POST['magasin_dest']) 		: $magasin_dest = '');
		//(isset($_POST['camion']) && $_POST['camion']!=''  				? $camion 		 = trim($_POST['camion']) 			: $camion = '');
		(isset($_POST['libelleetat']) && $_POST['libelleetat']!=''  	? $libelleetat 	 = trim($_POST['libelleetat']) 		: $libelleetat = '');
		...
		$_SESSION['DATA_TRS']['nbreLigne'] +=1;
		//print_r($_SESSION['DATA_DOT']);
		header('location:edittransfert2.php?selectedTab=bds');
		break;
	...
	case 'edit':
		...
		if($_SESSION['DATA_TRS']['nature']==1){header('location:edittransfert1.php?selectedTab=bds&rst=1');}
		elseif($_SESSION['DATA_TRS']['nature']==2){header('location:edittransfert2.php?selectedTab=bds&rst=1');}
		break;
	...
	case 'validate':
		...
		//TRANSFERT
		$sql = "SELECT * FROM  `transfert` WHERE (CODE_MAGASIN LIKE '". $_SESSION['GL_USER']['MAGASIN']."' OR  MAG_CODE_MAGASIN
		LIKE '". $_SESSION['GL_USER']['MAGASIN']."') AND   `CODE_TRANSFERT` LIKE '".addslashes($split[0])."'";
		$query =  $cnx->prepare($sql); //Prepare the SQL
		$query->execute(); //Execute prepared SQL => $query
		$row = $query->fetch(PDO::FETCH_ASSOC);
		...
		$_SESSION['DATA_TRS']['nbreLigne'] = $query->rowCount();
		if($_SESSION['DATA_TRS']['nature']==1) {header('location:validtransfert1.php?selectedTab=bds&rst=1');}
		elseif($_SESSION['DATA_TRS']['nature']==2) {header('location:validtransfert2.php?selectedTab=bds&rst=1');}
		break;
