...
function nombreCategorie($where=''){
...
function ligneConCategorie($wh='', $ord='', $sens='ASC', $page=1, $nelt){
...
$ilang=getCodelangue($userName);
	...
	//Nombre d'éléments
	$returnTble['NE'] = nombreCategorie($where);
	if($returnTble['NE']>0){
		//Calcule des limites
		$i = ($page-1)*$nelt;
		$sql = "SELECT * FROM categorie $where $order LIMIT $i, $nelt;";
		//Exécution
		try {
			$cnx = new PDO(DBD, DBUSER, DBPWD, array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8")); //Connexion to database
		}
		...
		while($row = $query->fetch(PDO::FETCH_ASSOC)){
			...
			<tr align="left" valign="middle" class="'.$col.'">
	            <td><input type="checkbox" name="rowSelection[]" value="'.$row['CODE_CATEGORIE'].'@'.$Use.'"></td>
...
function ligneEtatListeCategorie($wh=''){
$userName = getField('LOGIN',$_SESSION['GL_USER']['LOGIN'],'LOGIN','compte');
$ilang=getCodelangue($userName);
	$returnHTML = '';
	...
	(isset($ord) and $ord!=''  ? $order = " ORDER BY $ord $sens" : $order = " ORDER BY CODE_CATEGORIE ASC");
	//Nombre d'éléments
	$nbre = nombreCategorie($where);
	if($nbre>0){
		//Calcule des limites
		$sql = "SELECT * FROM categorie $where $order;";
		//Exécution
		try {
			$cnx = new PDO(DBD, DBUSER, DBPWD, array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8")); //Connexion to database
		}
		...
		while($row = $query->fetch(PDO::FETCH_ASSOC)){
			...
			<tr align="left" valign="middle">
                ...
                <td class="botBorderTdall"  align="center">'.(stripslashes($row['CODE_CATEGORIE'])).'&nbsp;</td>
