		...
		case 'nextlot':
			...
			if(count($produit)==0 ){
				...
				if ($categorie=='TOUS'){
					...
					//Produit
					$sql = "SELECT CODE_PRODUIT FROM produit INNER JOIN souscategorie ON (produit.CODE_SOUSCATEGORIE LIKE souscategorie.CODE_SOUSCATEGORIE)
					WHERE souscategorie.CODE_CATEGORIE LIKE '$categorie' ORDER BY PRD_LIBELLE ASC;";
					$query =  $cnx->prepare($sql); //Prepare the SQL
					$query->execute(); //Execute prepared SQL => $query
					while($row = $query->fetch(PDO::FETCH_ASSOC)){
...
if($ilang=='3' && $ilang!='') { $_SESSION['DATA_ETAT']['choix'] = 'Todos os sítios (Nacional)';}
			...
			if(count($produit)==0 ){
				...
				if ($categorie=='TOUS'){
					...
					//Produit
					$sql = "SELECT CODE_PRODUIT FROM produit INNER JOIN souscategorie ON (produit.CODE_SOUSCATEGORIE LIKE souscategorie.CODE_SOUSCATEGORIE) WHERE souscategorie.CODE_CATEGORIE LIKE '$categorie' ORDER BY PRD_LIBELLE ASC";
					$query =  $cnx->prepare($sql); //Prepare the SQL
					$query->execute(); //Execute prepared SQL => $query
					while($row = $query->fetch(PDO::FETCH_ASSOC)){
						$in .="'".$row['CODE_PRODUIT']."', ";
...
//			break;
		...
		case 'rapprdperime':
			...
			$sql = "SELECT * FROM mouvement INNER JOIN produit ON (mouvement.CODE_PRODUIT LIKE produit.CODE_PRODUIT)
			WHERE mouvement.MVT_TYPE LIKE 'E' AND  MVT_DATEPEREMP<NOW() $in GROUP BY mouvement.MVT_REFLOT ORDER BY produit.PRD_LIBELLE ASC; ";
			$query =  $cnx->prepare($sql); //Prepare the SQL
			$query->execute(); //Execute prepared SQL => $query
			$dp=date('Y-m-d');
			...
			while($row = $query->fetch(PDO::FETCH_ASSOC)){
				...
				$tperimeE = StockLotPerime($row['MVT_REFLOT'], $type='E', $whereAll." AND mouvement.MVT_DATEPEREMP<'".$dp."'");
				$tperimeS = StockLotPerimeS($row['MVT_REFLOT'], $type='S', $whereAll." AND mouvement.MVT_DATEPEREMP<'".$dp."'");
				$qteperimeS = $tperimeS['QTE'];
				$qteperimeE = $tperimeE['QTE'];
				$rest 	 = $qteperimeE - $qteperimeS;
			
				array_push($_SESSION['DATA_ETAT']['ligne'], array('codeproduit'=>$row['CODE_PRODUIT'], 'reflot'=>$row['MVT_REFLOT'],'produit'=>addslashes($row['PRD_LIBELLE']),
				'dateperemp'=>$row['MVT_DATEPEREMP'],'qte'=>$rest,'dateentree'=>$row['MVT_DATE'],'unite'=>$row['ID_UNITE']));
	...
	(isset($_POST['datefin']) && $_POST['datefin']!=''  ? $where .="mouvement.MVT_DATE <= '".addslashes(mysqlFormat(trim($_POST['datefin'])))."' AND " 	: $where .="");
			...
			while($row = $query->fetch(PDO::FETCH_ASSOC)){
				...
				if ($rest!=0 && $rest>$max) {
					...
					'qteentre'=>$entree,'transfertsort'=>$transfetSort['QTE'], 'transfertent'=>$transfetEnt['QTE'], 'bonsortie'=>$bonsortie['QTE'],
					'qteperime'=>$qeperime, 'reportentree'=>$reportEntree['QTE'], 'reportsortie'=>$reportSortie['QTE'],'declass'=>$Declassement['QTE'],
					'qtesortie'=>$sortie, 'ecart'=>$ecart,  'stocks'=>$rest,  'moisdispo'=>$moisdispo,  'max'=>$max,'unite'=>$row['ID_UNITE'],'cmm'=>$cmm));
					$Nbre++;
				}
