...
function nombreNature($where=''){
	$sql = "SELECT * FROM nature $where;";
	try {
...
function ligneConNature($wh='', $ord='', $sens='ASC', $page=1, $nelt){
...
$ilang=getCodelangue($userName);
	...
	//Oerder condition
	$order ='';
	(isset($ord) and $ord!=''  ? $order = " ORDER BY $ord $sens" : $order = " ORDER BY CODE_NATURE ASC");
	//Nombre d'éléments
	$returnTble['NE'] = nombreNature($where);
	if($returnTble['NE']>0){
		//Calcule des limites
		$i = ($page-1)*$nelt;
		$sql = "SELECT * FROM nature $where $order LIMIT $i, $nelt;";
		//Exécution
		try {
			$cnx = new PDO(DBD, DBUSER, DBPWD, array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8")); //Connexion to database
		}
		...
		while($row = 	$query->fetch(PDO::FETCH_ASSOC)){
			...
			<tr align="left" valign="middle" class="'.$col.'">
	            <td><input type="checkbox" name="rowSelection[]" value="'.$row['CODE_NATURE'].'" onClick="go('.$row['CODE_NATURE'].','.$j.');"></td>
                <td class="text" align="center">'.$row['CODE_NATURE'].'</td>
                <td class="text" >'.(stripslashes($row['SENS_MVT'])).'</td>
				<td class="text" >'.(stripslashes($row['NAT_LIBELLE'])).'</td>
