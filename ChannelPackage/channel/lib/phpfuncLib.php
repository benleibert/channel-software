...
//Application
define('TITLE', 'Channel2 Version 1.0-(Système de distribution des produits de santé)');
define('DEFAULTVIEWLENGTH', 20);

//------------------------------------ Stanadrd functions -----------------------------------
function getSiteCent($id)
{
$sql = '';
$sql .= "SELECT REGION FROM `region` WHERE IDREGION LIKE '$id';" ;
...
function getSiteFour($id)
{
$sql = '';
$sql .= "SELECT PROVINCE FROM `province` WHERE IDPROVINCE LIKE '$id';" ;
...
function selectProvinceC($login,$default){
...
$list='';
	...
	WHERE LOGIN LIKE '$login' AND SER_NOM!='$default' ORDER BY magasin.SER_NOM ASC;";
	try {
		$cnx = new PDO(DBD, DBUSER, DBPWD, array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8")); //Connexion to database
	}
	catch (PDOException $error) { //Treat error
...
function StockLotPerimeS($lot='', $type='S', $where=''){
	$sql = "SELECT `CODE_PRODUIT`,`ID_SOURCE`,`MVT_NATURE`, `MVT_VALID`, `MVT_UNITE`, MVT_DATEPEREMP, sum(`MVT_QUANTITE`) as QTE FROM mouvement
	 WHERE MVT_VALID=1 AND `MVT_TYPE` LIKE '$type' AND MVT_DATEPEREMP<NOW()  $where  group by `MVT_REFLOT` having `MVT_REFLOT`='$lot'; ";
...
function StockProduitDeclassePeriode($prd='', $type='', $d1='', $d2='', $where=''){
	$sql = "SELECT `CODE_PRODUIT`,`ID_SOURCE`,`MVT_NATURE`, `MVT_VALID`, `MVT_UNITE`, MVT_DATEPEREMP, sum(`MVT_QUANTITE`) as QTE FROM mouvement
	 WHERE MVT_VALID=1 AND MVT_TYPE LIKE '$type' AND MVT_NATURE LIKE 'PERTE' AND MVT_DATE>='$d1' AND MVT_DATE<='$d2'
...
$ilang=getCodelangue($userName);
	...
	while($row = $query->fetch(PDO::FETCH_ASSOC)){
		...
		$NVDeclassement = StockLotParNatureNonValide($row['MVT_REFLOT'], 'PERTE', $whereAll);
...
$ilang=getCodelangue($userName);
	...
	$Declassement = StockLotParNature($lot, 'PERTE', $whereAll);
	...
	//Declassement
	$PDeclassement = StockLotParNature($lot, 'PERTE', $whereAll);

	$NVbonsortie = StockLotParNatureNonValide($lot, 'BON DE SORTIE', $whereAll);
	$NVDeclassement = StockLotParNatureNonValide($lot, 'PERTE', $whereAll);
...
$ilang=getCodelangue($userName);
	...
	FROM  mouvement INNER JOIN produit ON (mouvement.CODE_PRODUIT LIKE produit.CODE_PRODUIT)
	GROUP BY mouvement.CODE_PRODUIT ORDER BY mouvement.CODE_PRODUIT ASC;";
...
function sousMenuDroitSansRech($page='', $tab='', $droit=array()){
$userName = getField('LOGIN',$_SESSION['GL_USER']['LOGIN'],'LOGIN','compte');
$ilang=getCodelangue($userName);
	if($ilang=='1' && $ilang!='') { 
	$return ='
...
function selectAnalyse($default=''){
	$da = array('1'=>'LIVRAISONS', '2'=>'AUTRES LIVRAISONS', '3'=>'DOTATIONS DES ETABLISSEMENTS', '4'=>'DOTATIONS BAC', '5'=>'DOTATIONS USTENSILES',
				'6'=>'AUTRES DOTATIONS','7'=>'PERTES', '8'=>'RECONDITIONNEMENTS','9'=>'REPORTS', '10'=>'TRANSFERTS');
...
//LOCALITE
function selectProvince($default=''){
	$sql = "SELECT * FROM province ORDER BY PROVINCE ASC;";
	try {
		$cnx = new PDO(DBD, DBUSER, DBPWD, array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8")); //Connexion to database
	}
